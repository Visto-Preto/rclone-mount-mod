...
PATH=/system/bin:/data/adb/magisk/busybox:/system/xbin
...
ln -sf ${HOME}/rclone /system/xbin/rclone
ln -sf ${HOME}/fusermount /system/xbin/fusermount3
ln -sf ${HOME}/rclonew /system/xbin/rclonew
...
custom_params () {
    ...
    if [[ ${remote} = global ]]; then
        ...
        PARAMS="DISABLE LOGFILE LOGLEVEL CACHEMODE CHUNKSIZE CHUNKTOTAL CACHEWORKERS CACHEINFOAGE DIRCACHETIME ATTRTIMEOUT BUFFERSIZE READAHEAD M_UID M_GID DIRPERMS FILEPERMS READONLY BINDSD ADD_PARAMS REPLACE_PARAMS NETCHK NETCHK_IF NETCHK_ADDR HTTP FTP HTTP_ADDR FTP_ADDR SFTP SFTP_ADDR SFTP_USER SFTP_PASS DLNA DLNA_ADDR DLNA_NAME PROFILE ISOLATE"
...
if [[ ${HTTP} = 1 ]]; then
    ...
    if $(/system/xbin/rclone serve http ${CLOUDROOTMOUNTPOINT} --addr ${HTTP_ADDR} --no-checksum --no-modtime --read-only >> /dev/null 2>&1 &); then
...
if [[ ${FTP} = 1 ]]; then
    ...
    if $(/system/xbin/rclone serve ftp ${CLOUDROOTMOUNTPOINT} --addr ${FTP_ADDR} --no-checksum --no-modtime --read-only >> /dev/null 2>&1 &); then
...
if [ ${DLNA} = 1 ] ; then
    ...
    if $(/system/xbin/rclone serve dlna ${CLOUDROOTMOUNTPOINT} --addr ${DLNA_ADDR} --no-checksum --no-modtime --read-only --name ${DLNA_NAME} >> /dev/null 2>&1 &); then

        echo "Notice: /mnt/cloud served via DLNA at: ${DLNA_ADDR}, check VLC -> Local Network to view..."
