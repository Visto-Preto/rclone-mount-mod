...
print_modname() {
  ...
  ui_print "+ $(curl -k https://beta.rclone.org/version.txt)"
  ui_print "*******************************"
}
...
on_install() {
  ...
  if [ "$ARCH" == "arm" ];then
    ...
    curl -k "https://beta.rclone.org/test/testbuilds-latest/rclone-android-16-armv7a.gz" | gunzip -d - > "$MODPATH"/rclone
  elif [ "$ARCH" == "arm64" ];then
    ui_print "+ downloading rclone-$ARCH to $MODPATH/rclone"
    curl -k "https://beta.rclone.org/test/testbuilds-latest/rclone-android-21-armv8a.gz" | gunzip -d - > "$MODPATH"/rclone
  elif [ "$ARCH" == "x86" ];then
    curl -k "https://beta.rclone.org/test/testbuilds-latest/rclone-android-16-x86.gz" | gunzip -d - > "$MODPATH"/rclone
  elif [ "$ARCH" == "x64" ];then
    curl -k "https://beta.rclone.org/test/testbuilds-latest/rclone-android-21-x64.gz" | gunzip -d - > "$MODPATH"/rclone
  fi
...
set_permissions() {
  ...
  ln -sf $MODPATH/fusermount /system/xbin/fusermount3
